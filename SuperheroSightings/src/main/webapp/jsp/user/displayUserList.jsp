...
<html>
    ...
    <body>
        ...
        <div class="container">
            ...
            <a class="text-center" href="displayUserForm"><button type="button" class="btn btn-default">Add a User</button></a>
            </div>
            <table class="table table-hover">
                <tr>
                    <th colspan="2" class="text-center bg-info text-white">Users</th>
                </tr>
                <tr>
                    <th class="text-center bg-info text-white">Username</th>
                    <th class="text-center bg-info text-white"></th>
                </tr>
                <c:forEach var="user" items="${users}">
                    <tr>
                        <td class="border-bottom border-info text-center">
                            <c:out value="${user.username}"/>
                        </td>
                        <td class="border-bottom border-info text-center">
                            <a href="${pageContext.request.contextPath}/user/deleteUser?userid=${user.id}">Delete</a> | 
                            <a href="${pageContext.request.contextPath}/user/editUserForm?userid=${user.id}">Edit</a>
                        </td>
                    </tr>
                </c:forEach>
            </table>
